...
export default function ChildPropertyForm({ formData, onInputChange, onSubmit, onClose, parentProperties, floorError }) {
  // Get the selected parent property
  const selectedParent = parentProperties.find((p) => p.id === parseInt(formData.property_id));

  // Generate floor options based on the selected parent's numberOfFloors
  const floorOptions = selectedParent ? Array.from({ length: selectedParent.numberOfFloors }, (_, i) => i + 1) : [];
          ...
          <div className="grid grid-cols-2 gap-6">
            ...
            <div>
              ...
              {formData.property_id ? (
                <select name="floor" value={formData.floor} onChange={onInputChange} className={formInputStyle} required>
