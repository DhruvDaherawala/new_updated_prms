...
export default function AllocationDetailModal({ allocation, onClose, refreshAllocations, apiUrl, renters, properties, childProperties }) {
  ...
  // Set available floors when component mounts or when property changes
  useEffect(() => {
    if (localAllocation.property_id || localAllocation.propertyId) {
      updateFloorOptions(localAllocation.property_id || localAllocation.propertyId);
    }
  }, [localAllocation.property_id, localAllocation.propertyId]);

  // Update floor options when property is selected
  const updateFloorOptions = (propertyId) => {
    const selectedProperty = properties.find((prop) => (prop.id || prop.property_id) == propertyId);

    if (selectedProperty && selectedProperty.numberOfFloors) {
      const floorCount = parseInt(selectedProperty.numberOfFloors);
  ...
  const handleAllocationChange = (field, value) => {
    ...
    // Update floor options when property is selected
    if (field === 'property_id' || field === 'propertyId') {
      updateFloorOptions(value);
    }
  };
  ...
  const saveAllocation = async () => {
    try {
      ...
      const dataToSend = {
        renter_id: localAllocation.renter_id || localAllocation.renterId,
        property_id: localAllocation.property_id || localAllocation.propertyId,
        floor_number: localAllocation.numberOfFloors || '',
        allocation_date: localAllocation.allocation_date || localAllocation.startDate,
        remarks: localAllocation.remarks,
        status: localAllocation.status || 'Active'
      };
      ...
      alert('Allocation updated successfully!');
      console.log('Edited data', ApiService.updateAllocation);
      refreshAllocations();
      setIsEditing(false);
    } catch (error) {
      alert('Failed to update allocation!');
        ...
        <div className="grid gap-1">
          <div className="space-y-4">
            ...
            <div>
              ...
              {isEditing ? (
                <select
                  value={localAllocation.numberOfFloors || ''}
                  onChange={(e) => handleAllocationChange('floor_number', e.target.value)}
                  className={Styles.formInputStyle}
                ...
                <p className={Styles.cardStyle}>{localAllocation.floor_number ? `Floor ${localAllocation.floor_number}` : 'Not set'}</p>
              )}
            </div>
            <div>
              <label className="font-semibold">Allocation Date:</label>
