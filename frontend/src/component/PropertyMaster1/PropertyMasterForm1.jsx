...
export default function PropertyMasterForm() {
  ...
  const filteredProperties = properties.filter(
    (property) => {
      // If searching specifically for status values
      ...
      return property.propertyName?.toLowerCase().includes(searchTerm.toLowerCase()) ||
  ...
  return (
    <Container maxWidth="lg" sx={{ py: 4 }}>
      ...
      <Dialog open={showAddPropertyModal} onClose={() => setShowAddPropertyModal(false)} fullWidth maxWidth="md">
        ...
        <DialogActions>
          <Button onClick={() => setShowAddPropertyModal(false)} startIcon={<CancelIcon />}>Cancel</Button>
          <Button onClick={handleSubmit} startIcon={<SaveIcon />} variant="contained" color="primary">Save</Button>
        </DialogActions>
      </Dialog>
